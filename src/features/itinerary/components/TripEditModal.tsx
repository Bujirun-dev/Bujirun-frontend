...
export function TripEditModal({ isOpen, trip, onClose, onConfirm }: TripEditModalProps) {
  ...
  return (
    <Modal
      ...
      className="gap-[24px] px-[34px] py-[40px]"
    ...
    >
      <div className="flex flex-col gap-3">
        <div className="flex items-center gap-[7px]">
          ...
          <span className="font-paperlogy text-[16px] font-semibold text-text-heading">여행명</span>
        </div>
        <TextInput
          value={name}
          onChange={(event) => setName(event.target.value)}
          placeholder="여행 이름을 입력하세요"
          className="h-[30px] w-[275px] rounded-[10px] border border-main-blue text-[11px] font-medium text-text-primary"
        />
      </div>
      ...
      <div className="flex flex-col gap-[8px]">
        <div className="flex items-center gap-[7px]">
          ...
          <span className="font-paperlogy text-[16px] font-semibold text-text-heading">
            여행 기간
          </span>
        </div>
        ...
        <Card variant="glass-sm" className="mt-[20px] w-full rounded-[10px] px-3 py-2">
...
function DateTimeLabel({ label }: { label: string }) {
  return (
    <div className="flex shrink-0 items-center gap-[5px]">
      ...
      <span className="font-paperlogy text-[12px] font-semibold text-text-primary">{label}</span>
    </div>
  );
}
