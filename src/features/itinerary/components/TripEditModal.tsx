    ...
    >
      <div className="flex flex-col gap-3">
        <div className="flex items-center gap-1.5">
          ...
          <span className="font-paperlogy text-lg font-semibold text-text-heading">여행명</span>
        </div>
        <TextInput
          value={name}
...
function DateTimeLabel({ label }: { label: string }) {
  return (
    <div className="flex shrink-0 items-center gap-1">
      ...
      <span className="font-paperlogy text-sm font-semibold text-text-primary">{label}</span>
    </div>
  );
}
