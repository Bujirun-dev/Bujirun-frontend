...
export function CollaboratorBadge({ editors }: { editors: CollaboratorInfo[] }) {
  ...
  return (
    <div className="flex items-center -space-x-1">
      {visible.map((editor, idx) => {
        ...
        return (
          <div
            key={`${editor.name}-${idx}`}
            className={cn(
              "relative flex size-8 items-center justify-center overflow-hidden rounded-full border border-main-white",
              editor.colorClass,
            )}
            title={editor.name}
          >
            {avatarSrc ? (
              <Image src={avatarSrc} alt={editor.name} fill sizes="32px" className="object-cover" />
            ) : (
              <span className="text-xs font-semibold text-main-white">
      ...
      {overflow > 0 && (
        <div className="flex size-8 items-center justify-center rounded-full border border-main-white bg-sub-gray">
          <span className="text-xs font-semibold text-main-white">+{overflow}</span>
        </div>
      )}
    </div>
  );
