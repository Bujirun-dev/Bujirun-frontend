...
export function TransportLegItem({ leg, metaText, className }: TransportLegItemProps) {
  ...
  return (
    <div className={cn("flex min-w-0 flex-1 flex-col gap-0.5", className)}>
      ...
      {leg.arrivalText && shouldShowArrival ? (
        <div
          className={cn(
            "my-2 flex items-center justify-between rounded-[10px] px-3 py-2 text-md font-semibold text-main-white",
            ARRIVAL_COLORS[leg.type as (typeof ARRIVAL_VISIBLE_TYPES)[number]],
          )}
        >
          <span>{leg.arrivalText}</span>
          <svg viewBox="0 0 512 512" className="size-3.5 fill-main-white" aria-hidden="true">
