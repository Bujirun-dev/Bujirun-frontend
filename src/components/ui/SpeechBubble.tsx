...
const speechBubbleVariants: Record<SpeechBubbleVariant, { bubble: string; tailColor: string; dropShadow: string }> = {
  white: {
...
export function SpeechBubble({
  ...
  const tailStyle = {
    ...
    ...(isBottom
      ? { borderLeft: "6px solid transparent", borderRight: "6px solid transparent", borderTop: `6px solid ${tailColor}` }
      : { borderLeft: "6px solid transparent", borderRight: "6px solid transparent", borderBottom: `6px solid ${tailColor}` }),
  };

  return (
    <div className={cn("relative", isBottom ? "pb-1.5" : "pt-1.5", tailCenter && "w-fit", className)} style={{ filter: dropShadow }}>
